 ...
 * Trades are dots whose diameter grows with log(size).
 */
...
const TRADE_COLOR: RGBA = [0.1, 0.35, 0.85, 0.45];
...
function init() {
  ...
  function pushToGpu(e: QuoteEvent): void {
    if (e.kind === "trade") {
      const radius = (1 + 1.25 * Math.log10(e.size)) * dpr;
      trades.addDot(xs(e.t), e.price, radius, TRADE_COLOR);
      return;
    }
  ...
  function rebuildGpu(): void {
    quotes.clear();
    trades.clear();
    for (const e of events) pushToGpu(e);
  }
  ...
  function animate(ts: number): void {
    if (ts - lastDraw >= REDRAW_MS) {
      ...
      stats.textContent = `${quotes.numSegments} quotes, ${trades.numDots} trades in window`;
    }
